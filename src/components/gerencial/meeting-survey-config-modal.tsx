...
import { MEETING_DEFAULTS } from "@/lib/data/meeting-survey";
import type { NpsQuestion } from "@/lib/data/nps";
...
/** Edita textos + perguntas extras da pesquisa pós-reunião (escala 1–5 é fixa). */
export function MeetingSurveyConfigModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    fetch("/api/gerencial/meeting-survey-config", { cache: "no-store" })
  ...
  async function save() {
    setBusy(true);
    try {
      const res = await fetch("/api/gerencial/meeting-survey-config", {
        method: "POST",
      ...
      if (!res.ok) toast(j?.error ?? "Não foi possível salvar.", "error");
        ...
        toast("Pesquisa pós-reunião salva.", "success");
        onClose();
      }
    } finally {
      setBusy(false);
  ...
  return (
    <Modal
      open
      onClose={onClose}
      title="Personalizar pesquisa pós-reunião"
      description="Edite os textos e adicione perguntas extras. A avaliação de 1 a 5 estrelas é fixa."
      size="lg"
      footer={
        ...
        <div className="space-y-4">
          <div className="space-y-3">
            <label className="block">
              <span className={labelCls}>Pergunta principal (estrelas)</span>
              <textarea value={f.headline} onChange={(e) => set("headline", e.target.value)} rows={2} placeholder={MEETING_DEFAULTS.headline} className={inputCls + " resize-y"} />
            </label>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <label className="block">
                <span className={labelCls}>Introdução (subtítulo)</span>
                <input value={f.intro} onChange={(e) => set("intro", e.target.value)} placeholder="Padrão: Pesquisa pós-reunião · <cliente>" className={inputCls} />
              </label>
              <label className="block">
                <span className={labelCls}>Rótulo do comentário</span>
                <input value={f.commentLabel} onChange={(e) => set("commentLabel", e.target.value)} placeholder={MEETING_DEFAULTS.commentLabel} className={inputCls} />
              </label>
            </div>
            <label className="block">
          ...
          <div className="border-t border-line pt-4">
            ...
            {questions.length === 0 ? (
              <p className="rounded-lg bg-subtle px-3 py-2 text-xs text-muted">Sem perguntas extras. A pesquisa terá só as estrelas e o comentário.</p>
            ) : (
                  ...
                  <div key={q.key} className="rounded-xl border border-line p-2.5">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0 flex-1 space-y-2">
                        <input value={q.label} onChange={(e) => setQ(q.key, { label: e.target.value })} placeholder="Texto da pergunta (ex.: O que faltou na reunião?)" className={inputCls} />
                        <div className="flex flex-wrap items-center gap-2">
                          <select value={q.type} onChange={(e) => setQ(q.key, { type: e.target.value as NpsQuestion["type"] })} className="rounded-lg border border-line bg-surface px-2 py-1 text-xs text-ink outline-none focus:border-brand-400">
                            <option value="text">Resposta livre</option>
                            <option value="choice">Escolha única</option>
                          </select>
                          {q.type === "choice" && (
                            <input value={q.options.join(", ")} onChange={(e) => setQ(q.key, { options: e.target.value.split(",").map((x) => x.trim()).filter(Boolean) })} placeholder="Opções separadas por vírgula" className="min-w-0 flex-1 rounded-lg border border-line bg-surface px-2 py-1 text-xs text-ink outline-none focus:border-brand-400" />
                          )}
                        </div>
                      </div>
