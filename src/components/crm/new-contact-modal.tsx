...
import type { Company } from "@/lib/data/crm";
      ...
      <div className="relative z-10 max-h-[90vh] w-full max-w-md overflow-y-auto rounded-2xl border border-line bg-surface shadow-2xl">
        <div className="flex items-start justify-between gap-3 border-b border-line p-5">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold text-ink">Novo contato</h2>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 p-5">
          ...
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-muted">Cargo</span>
            <input value={f.title} onChange={(e) => set("title", e.target.value)} className={inputCls} />
          </label>
          <label className="block">
            <span className="mb-1 block text-xs font-medium text-muted">Telefone (WhatsApp)</span>
            <input value={f.phone} onChange={(e) => set("phone", e.target.value)} inputMode="tel" placeholder="5527999998888" className={inputCls} />
          </label>
          <label className="col-span-2 block">
