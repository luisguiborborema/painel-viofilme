...
/** Todas as respostas da pesquisa pós-reunião (gerência). */
export async function sbGetAllMeetingSurveys(): Promise<import("./meeting-survey").MeetingEntry[]> {
  const supabase = await createClient();
  const { data } = await supabase
    .from("meeting_surveys")
    .select("id, client_id, rating, comment, extra, respondent, channel, created_at, answered_at, clients(name)")
    .not("rating", "is", null)
    .order("created_at", { ascending: false })
    .limit(1000);
  return ((data ?? []) as Record<string, unknown>[]).map((r) => {
...
export async function sbGetDeliveryTasks(): Promise<DeliveryTask[]> {
  ...
  const { data } = await supabase
    .from("delivery_tasks")
    .select(
      "id, title, type, origin, assignee, stage, due_date, estimate_h, logged_h, urgent, checklist, comments, priority, assignees, requester, moved_at, created_at, completed_at, custom_fields, campaign_goal, content_format, duration_min, tema, roteiro, legenda, refs, post_date_iso, delivery_date, delivery_overridden, commemorative_date, clients(name)",
    )
...
function mapCrmLead(r: CrmLeadRow): CrmLead {
  ...
  return {
    ...
    expectedCloseAt: s("expected_close_at"),
...
export async function sbGetCrmPipelines(): Promise<Pipeline[]> {
  ...
  const [{ data: pipes }, { data: stages }] = await Promise.all([
    ...
    supabase
      .from("crm_stages")
      .select("id,pipeline_id,key,label,color,probability,position,kind,is_handoff,hint,requirements,automations")
      .order("position", { ascending: true }),
  ]);
  return (pipes ?? []).map((p) => ({
    id: String(p.id),
    ...
    stages: (stages ?? [])
      ...
      .map(
        (s): Stage => ({
          ...
          isHandoff: Boolean(s.is_handoff),
...
export async function sbGetCrmReports(): Promise<ReportDef[]> {
  const supabase = await createClient();
  const { data } = await supabase.from("crm_settings").select("value").eq("key", "reports").maybeSingle();
  const v = data?.value as { reports?: unknown } | null;
  const reports = Array.isArray(v?.reports) ? (v!.reports as ReportDef[]) : [];
  return reports.filter((r) => r && typeof r.id === "string");
}

export async function sbGetCrmDashboards(): Promise<DashboardDef[]> {
  const supabase = await createClient();
  const { data } = await supabase.from("crm_settings").select("value").eq("key", "reports").maybeSingle();
  const v = data?.value as { dashboards?: unknown } | null;
  const dashboards = Array.isArray(v?.dashboards) ? (v!.dashboards as DashboardDef[]) : [];
  return dashboards.filter((d) => d && typeof d.id === "string");
}
